-- See https://github.com/YieldingExploiter/Scripts/blob/main/src/BadBuisness/Script.lua
return (function()
  ...
  local function myCharacter()
    for i, v in ipairs(workspace.Characters:GetChildren()) do
      if (v.Body.Head.Position - camera.CFrame.Position).magnitude < 1 then return v end
    end
  end

  local function onTeam( x )
    for i, v in ipairs(client.PlayerGui:GetChildren()) do
      if v.Name == 'NameGui' then if v.Adornee:IsDescendantOf(x) then return true end end
    end
  end
      ...
      if v ~= myCharacter() and myCharacter() and not onTeam(v) then
        ...
        if onscreen then
          ...
          local dist = (Vector2.new(mousePos.X, mousePos.Y) -
                         Vector2.new(targetPos.X, targetPos.Y)).magnitude
  ...
  silentFovObject.Transparency = _G.BBConfig.fovTransparency
  ...
  gmt.__namecall = newcclosure(
    function( self, ... )
      ...
      if tostring(self) == 'Projectiles' and tostring(method) == 'FireServer' and
        closestPlayer(_G.BBconfig.fovAmount) and silentAimEnabled then
  ...
  local flySettings = {
    ...
    enabled = false;
    ['mf'] = {
      [true] = Vector3.new(math.huge, math.huge, math.huge);
      [false] = Vector3.new();
    };
    ...
    for i, v in ipairs(boxes) do
      if not v['Character']:IsDescendantOf(workspace) then
        ...
        local BL = camera:WorldToViewportPoint(
          (character.Root.CFrame * CFrame.new(-2, -5, 0)).p
        )
    ...
    function()
      ...
      if myCharacter() then
        ...
        if newPos ~= character.Root.Position and newPos then
          ...
          velPart.Velocity = CFrame.new(character.Root.Position, newPos).LookVector *
                               flySettings.speed
      ...
      if isAiming and closestPlayer(_G.BBconfig.fovAmount) then
        aimAt(
          ...
          _G.BBconfig.aimbotSmoothness
        )
      end
      -- ANCHOR Silent Aim 
      silentFovObject.Color = _G.BBconfig.fovColor -- Live Update Config
      silentFovObject.Transparency = _G.BBConfig.fovTransparency -- Live Update Config
